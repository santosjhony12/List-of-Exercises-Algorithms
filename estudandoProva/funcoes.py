...
# near_ten(19) -> True
def near_ten(n):
  if(n%10==0 or n%10==1 or n%10==2 or n%10==8 or n%10==9):
    return True
  else:
    return False
...
def count_code(s):
  d = ["a","b","c","d","e","f", "g", "h", "i", "j", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "y", "z", ""]
  a = 0
...
def end_other(a, b):
   ...
   if a2.endswith(b2) or b2.endswith(a2):
        return True
   else:
        return False
...
def fila_tijolos(n_peq, n_gra, meta):
    return meta % 5 <= n_peq and meta <= (n_peq + n_gra * 5)
